...
import 'html/rrect_renderer.dart';
...
class CanvasPool extends _SaveStackTracking {
  ...
  void closeCanvas() {
    ...
    if (_canvas != null) {
      ...
      _activeCanvasList ??= <DomHTMLCanvasElement>[];
  ...
  DomHTMLCanvasElement? _allocCanvas(int width, int height) {
    // The dartdocs for `tryCreateCanvasElement` on why we don't use the
    // `DomHTMLCanvasElement` constructor.
  ...
  void endOfPaint() {
    if (_reusablePool != null) {
      for (final DomHTMLCanvasElement e in _reusablePool!) {
  ...
  void _resetTransform() {
    final DomHTMLCanvasElement? canvas = _canvas;
    if (canvas != null) {
  ...
  void _clearActiveCanvasList() {
    if (_activeCanvasList != null) {
      for (final DomHTMLCanvasElement c in _activeCanvasList!) {
        if (ui_web.browser.browserEngine == ui_web.BrowserEngine.webkit) {
          c.width = c.height = 0;
        }
        c.remove();
